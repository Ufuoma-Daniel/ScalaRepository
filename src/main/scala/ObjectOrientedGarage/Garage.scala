...
class Garage (a: String) {
  ...
  def addVehiclesTolist (newVList: ListBuffer[Vehicle]) { vehicleList += newVList}
  def addCarToVehicleList(newVehicle: Car) : Unit = {newVehicle.setVehicleID("C"+cIdTally); cIdTally+=1;carList += newVehicle}
  def addBikeToVehicleList(newVehicle: Bike) : Unit = {newVehicle.setVehicleID("B"+bIdTally); bIdTally+=1;bikeList += newVehicle}
  ...
  def printVehicleList() {
      for(x<-0 until carList.size) {println(carList(x).printDetails())}
      for(x<-0 until bikeList.size) {println(bikeList(x).printDetails())}
    }
  def calculateTotalFixTime() {
    var totalTime = 0;
    for(x<-0 until carList.size) { totalTime += carList(x).totalFixTime()}
    for(x<-0 until bikeList.size) { totalTime += bikeList(x).totalFixTime()}
    println("Time to fix all Vehicles: "+totalTime+ "seconds")
  }
  def calculateTotalCost() {
    var totalTime = 0;
    for(x<-0 until carList.size) { totalTime += carList(x).totalCost()}
    for(x<-0 until bikeList.size) { totalTime += bikeList(x).totalCost()}
  ...
  def printEmployeeList() {
    for(loop2<-0 until employeeList.size){println(employeeList(loop2).printEDetails())}}
